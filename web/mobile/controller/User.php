    ...
    public function login()
    {
        if(IS_POST)
        {
            try
            {
                ...
                $res = $m->getNewLoginData('username',$param['username'],$param['password'],'id,username,address,password');
                if(!$res)
                {
...
//                'invite_username|推荐人'  => ['require',]
            ...
            $mobile_count = $m->hasRegsterPhone($param['phone']);
            if($mobile_count >= 5)
            {
                return $this->failData('此手机号已经超过注册次数');
            }

            $count = $m->hasRegsterUsername($param['username']);
            if($count > 0)
            {
                return $this->failData('此用户名已被注册，请直接登录或尝试找回密码');
            }

            $invite_user_id = '';
            if(!empty($param['invite_username']))
            {
                $invite_user_id = $m->getUserByUsername($param['invite_username']);
            ...
            try{
                ...
                if(!empty($_verify))
                {
                    ...
                    if($res)
                    {
                        ...
                        $memberData['address']  = $user['address'];
                        session('memberData',$memberData);
...
//            $res['code'] = '1111';
    ...
    public function toForgetPpassword()
    {
    ...
    public function forgetPassword()
    {
        ...
        if(empty($user))
        {
            return $this->failData('此用户不存在');
        }

        $verfyM = new \addons\member\model\VericodeModel();
        $_verify = $verfyM->VerifyCode($verify_code,$phone);
        if(empty($_verify))
            return $this->failData('验证码失效，请重新获取');
