...
class OtcOrder extends \web\common\model\BaseModel{
    ...
    public function getOrderDetail($id){
        ...
        $sql = 'select a.*,b.username ,c.username as buy_username from '.$this->getTableName().' a left join '.$m->getTableName().' b on a.user_id=b.id left join '.$m->getTableName().' c on a.buy_user_id=c.id where a.id='.$id.' limit 0,1';
        $data = $this->query($sql);
        if(!empty(($data))){
            return $data[0];
        }else{
